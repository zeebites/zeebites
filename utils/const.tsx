export const COMPANY_PROFILE = {
	logo: '/icon-72x72.png',
	logolg: '/icon-512x512.png',
	name: 'Zeebites Delight',
	tagline: 'Delicious Moments, Unforgettable Events',
	address: '8015 Barnes Ridge Ln, Houston TX, 77072',
	phone: '+13467159206',
	email: 'zaidance4u@yahoo.co.uk',
	instagramHandle: 'zeebites_delight',
	calendly: 'https://calendly.com/zeebites',
	bio: `Zeebites is a leading catering service company established in 2020, providing delicious snacks and refreshments to customers all over the world. Our mission is to provide the highest quality snacks and refreshments to our customers, while also ensuring that our products are affordable and accessible. We offer a wide range of products, including puffpuff, chicken puff, moin-moin, chin-chin, spring roll, samosa’s and assorted turkey. Our team of experienced chefs and bakers use only the freshest ingredients and the latest cooking techniques to ensure that our products are of the highest quality. We are committed to providing our customers with the best possible experience and strive to exceed their expectations. We have achieved many milestones since our establishment, including becoming one of the most popular catering services in the world. We have also been featured in several international magazines and newspapers, and have been recognized for our commitment to excellence. At Zeebites, we strive to provide our customers with the best possible experience, and we are committed to providing the highest quality snacks and refreshments. We look forward to serving you with our delicious snacks and refreshments.`,
	bullets: [
		'Zeebites Delight is a leading catering service company established in 2020, providing delicious snacks and refreshments to customers all over the world. ',
		'Our mission is to provide the highest quality snacks and refreshments to our customers, while also ensuring that our products are affordable and accessible. ',
		'We offer a wide range of products, including puffpuff, chicken puff, moin-moin, chin-chin, spring roll, samosa’s and assorted turkey. ',
		'Our snacks and refreshments are made with the freshest ingredients, ensuring that our customers get the best quality. ',
		'We strive to provide our customers with the best possible experience, from the convenience of ordering to the taste of the snacks. ',
		'Our commitment to quality and affordability makes us the perfect choice for any catering needs.',
	],
	navlinks: [
		{ title: 'Home', url: '/' },
		{ title: 'About', url: '/#about' },
		{ title: 'Menu', url: '/#menu' },
		{ title: 'Testimonials', url: '/#testimonials' },
		{ title: 'Gallery', url: '/#gallery' },
		{ title: 'Pricing', url: '/#pricing' },
		{ title: 'Contact', url: '/#contact' },
	],
	heroTitle: 'Delight Your Taste Buds',
	heroSubtitle: 'Experience a World of Flavors',
	heroCTA: 'Book Your Catering Now',
	heroType: 'video', // or 'image'
	heroImage: '/samosas2.jpg',
	heroVideo: '/video.webm',
	valueProposition:
		'At Zeebites Delight, we create culinary magic to make your events unforgettable. With our handcrafted snacks and refreshments, we guarantee to satisfy your taste buds and leave a lasting impression on your guests.',
	services: [
		{
			name: 'Puffpuff',
			description: 'Delicious deep-fried dough balls.',
			image: '/puffpuff.jpg',
			items: ['Classic Puffpuff', 'Sweet Puffpuff', 'Savory Puffpuff'],
		},
		{
			name: 'Chicken Puff',
			description: 'Savory pastry filled with seasoned chicken.',
			image: '/chickenpuff.jpg',
			items: [
				'Spicy Chicken Puff',
				'Cheesy Chicken Puff',
				'BBQ Chicken Puff',
			],
		},
		{
			name: 'Moin-moin',
			description: 'Steamed bean cakes with a rich blend of spices.',
			image: '/moinmoin.jpg',
			items: [
				'Traditional Moin-moin',
				'Classic Moin-moin',
				'Fish/Beef Moin-moin',
			],
		},
		{
			name: 'Chin-chin',
			description: 'Crunchy, sweet, and lightly spiced snacks.',
			image: '/chinchin.jpg',
			items: [
				'Classic Chin-chin',
				'Garlic Chin-chin',
				'Cinnamon Chin-chin',
			],
		},
		{
			name: 'Spring Roll',
			description: 'Thin pastry filled with a variety of fillings.',
			image: '/springroll1.jpg',
			items: [
				'Vegetable Spring Roll',
				'Chicken Spring Roll',
				'Shrimp Spring Roll',
			],
		},
		{
			name: 'Samosas',
			description: 'Crispy pastry filled with savory fillings.',
			image: '/samosas4.jpg',
			items: ['Vegetable Samosa', 'Chicken Samosa', 'Beef Samosa'],
		},
		{
			name: 'Assorted Turkey',
			description: 'Tender and flavorful turkey meat.',
			image: '/turkey.jpg',
			items: ['Roasted Turkey', 'Spicy BBQ Turkey', 'Fried Turkey'],
		},
		{
			name: 'Kelewele (Ghanaian flavor)',
			description: 'Fried plantains seasoned with aromatic spices.',
			image: '/kelewele.jpg',
			items: [
				'Spicy Kelewele',
				'Sweet and Tangy Kelewele',
				'Coconut-Infused Kelewele',
			],
		},
	],
	testimonials: [
		{
			content: 'I wanted to surprise my cousin in Houston, Texas for her graduation ceremony, and Zeebites Delight made it possible. Despite the distance, the package arrived right on time, and the snacks were delicious! My cousin was thrilled, and it added an extra special touch to her celebration. Thank you, Zeebites Delight, for the fantastic service!',
			author: 'Emma from London, UK',
			role: 'Parent',
			avatar: '/avatar6.jpg',
		},
		{
			content: "Zeebites Delight catered our event, and their snacks were a huge hit! Our guests couldn't get enough of the delicious Puffpuff and Spring Rolls. Highly recommended!",
			author: 'Ogunbodede Lawal',
			role: 'Event Organizer',
			avatar: '/avatar1.jpg',
		},
		{
			content: "The Chin-chin from Zeebites Delight is absolutely addictive! Their attention to flavor and quality is unmatched. I can't wait to order again!",
			author: 'Chritiana Nwachukwu',
			role: 'Food Lover',
			avatar: '/avatar2.jpg',
		},
		{
			content: 'Moin-moin is one of my favorite Nigerian snacks, and Zeebites Delight does it perfectly! Their Moin-moin is always moist and full of flavor. Definitely ordering again!',
			author: 'Sarah Adetokumbo',
			role: 'Snack Enthusiast',
			avatar: '/avatar3.jpg',
		},
		{
			content: 'The Assorted Turkey from Zeebites Delight is a must-try! Tender and juicy, with incredible seasoning. It was the highlight of our Thanksgiving dinner. Thank you!',
			author: 'Michael Brown',
			role: 'Turkey Lover',
			avatar: '/avatar4.jpg',
		},
		{
			content: 'I recently discovered Kelewele from Zeebites Delight, and it has quickly become my go-to snack. The combination of spices is perfect, and the texture is divine. Highly recommended!',
			author: 'Olusola Wilson',
			role: 'Flavor Adventurer',
			avatar: '/avatar5.jpg',
		},
	],
	featuredEvents: [
		{
			title: 'Food Fest 2023',
			description: `Join us at Food Fest 2023, where we'll be showcasing our mouthwatering snacks and refreshments. Don\'t miss out on this culinary extravaganza!`,
			image: '/event1.jpg',
			link: '/#',
		},
		{
			title: 'Summer BBQ Bash',
			description:
				"Celebrate summer with our sizzling BBQ Bash! Indulge in our grilled delights and experience a flavor-packed event you won't forget.",
			image: '/event2.jpg',
			link: '/#',
		},
	],
	howItWorks: [
		{
			number: '1',
			title: 'Place Your Order',
			description:
				'Browse our menu, checkout your favorite snacks and refreshments, click book now to place your order on Whatsapp.',
		},
		{
			number: '2',
			title: 'We Prepare Your Delights',
			description:
				'Our skilled team of chefs will freshly prepare your snacks and refreshments with the finest ingredients.',
		},
		{
			number: '3',
			title: 'Delivery or Pickup',
			description:
				'Choose your preferred delivery option or schedule a pickup. Sit back and relax while we take care of the rest.',
		},
		{
			number: '4',
			title: 'Enjoy Your Delicious Moments',
			description:
				'Indulge in the delicious snacks and refreshments from Zeebites Delight, and create unforgettable moments.',
		},
	],
	packages: [
		{
			name: 'Essential Package',
			description:
				'Perfect for small gatherings and casual events. Includes a variety of snacks and refreshments.',
			price: 10,
			link: 'https://wa.link/ynskwn',
			plan: [
				'Puffpuff',
				'Samosa or Spring Rolls',
				'Chicken Puff',
				'Small Pack of Chin-chin',
			],
			minimum: 10,
		},
		{
			name: 'Deluxe Package 1',
			description:
				'Ideal for medium-sized events and parties. Offers an extensive selection of snacks and refreshments.',
			price: 12,
			link: 'https://wa.link/f2mlwg',
			plan: [
				'Puffpuff',
				'Samosa or Spring Rolls',
				'Moin Moin',
				'Fried Turkey',
				'Fried Plaintain',
			],
			minimum: 10,
		},
		{
			name: 'Deluxe Package 2',
			description:
				'Ideal for medium-sized events and parties. Offers an extensive selection of snacks and refreshments.',
			price: 12,
			link: 'https://wa.link/pabap7',
			plan: [
				'Puffpuff',
				'Samosa or Spring Rolls',
				'Moin Moin',
				'Fried Turkey',
				'Fried Plaintain',
			],
			minimum: 10,
		},
		{
			name: 'Premium Package 1',
			description:
				'Designed for large-scale events and special occasions. Features a wide range of snacks and refreshments.',
			price: 160,
			link: 'https://wa.link/sqogig',
			plan: [
				'Puffpuff (50pcs)',
				'Spring Rolls (20pcs)',
				'Samosa (20pcs)',
				'Gizzard in hot sauce (20pcs)',
				'Fried Turkey (5pcs)',
				'Prawn in butter (5pcs)',
				'Fried Plantain',
			],
			minimum: 'custom',
		},
		{
			name: 'Premium Package 2',
			description:
				'Designed for large-scale events and special occasions. Features a wide range of snacks and refreshments.',
			price: 160,
			link: 'https://wa.link/fb3zkt',
			plan: [
				'Puffpuff (50pcs)',
				'Spring Rolls (20pcs)',
				'Samosa (20pcs)',
				'Chicken Wings in hot sauce (15pcs)',
				'Gizzard in hot sauce (15pcs)',
				'Prawn in butter (5pcs)',
				'Fried Plantain',
			],
			minimum: 'custom',
		},
		{
			name: 'Custom Package',
			description:
				'Tailored to your specific needs and preferences. Contact us for a personalized package.',
			price: 'Calculated', // Set the price to 0 or specify a custom price
			link: 'https://wa.link/88kfxxe',
			plan: [
				'Choose your preferred snacks and refreshments',
				'Specify the quantities and any dietary preferences',
				'Get a personalized quote based on your selections',
			],
			minimum: 'custom',
		},
	],
	aboutUs: {
		image: '/about.jpg',
		title: 'Welcome to Zeebites Delight',
		description:
			'At Zeebites Delight, we are passionate about providing delicious snacks and refreshments for all occasions. With our wide range of mouthwatering treats, we aim to make every event a delightful experience.',
		mission: 'Our mission is to satisfy our customers with the best quality snacks and refreshments, ensuring each bite is filled with flavor and enjoyment.',
		vision: 'Our vision is to become the go-to choice for snacks and refreshments catering, delivering delicious moments and unforgettable events.',
	},
	faq: [
		{
			question: 'What snacks do you offer?',
			answer: 'We offer a variety of snacks including Puffpuff, Chicken Puff, Moin-moin, Chin-chin, Spring roll, Samosa’s, Assorted Turkey, and KELEWELE (Ghanaian flavour).',
		},
		{
			question: 'Do you cater to dietary preferences?',
			answer: 'Yes, we can accommodate dietary preferences such as vegetarian, vegan, gluten-free, and more. Please let us know your specific requirements when placing an order.',
		},
		// Add more FAQ items as needed
	],
	gallery: [
		{
			url: '/puffpuff.jpg',
			caption: 'Delicious Puffpuff',
			category: 'snacks',
		},
		{
			url: '/event1.jpg',
			caption: 'May Fiesta',
			category: 'events',
		},
		{
			url: '/chickenpuff.jpg',
			caption: 'Tasty Chicken Puff',
			category: 'snacks',
		},
		{
			url: '/event2.jpg',
			caption: 'Culinary Symphony',
			category: 'events',
		},
		{
			url: '/samosas4.jpg',
			caption: 'Superem Samosas',
			category: 'snacks',
		},
		{
			url: '/event3.jpg',
			caption: 'Food Haven',
			category: 'snacks',
		},
		{
			url: '/springroll1.jpg',
			caption: 'Super Springrolls',
			category: 'snacks',
		},
		{
			url: '/event4.jpg',
			caption: 'Barbecue Fiesta',
			category: 'snacks',
		},
		{
			url: '/about.jpg',
			caption: 'Event Setup',
			category: 'events',
		},
		// Add more gallery images with their respective categories
	],
	social: [
		{
			platform: 'Instagram',
			url: 'https://www.instagram.com/Zeebites_delight',
			icon: '/instagram.png',
		},
		// Add more social media platforms as needed
	],
	map: 'https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3465.9938342847277!2d-95.61365442432094!3d29.690958435232204!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x8640dd98fc971695%3A0x237fb12e3ffcfd4f!2s8015%20Barnes%20Ridge%20Ln%2C%20Houston%2C%20TX%2077072%2C%20USA!5e0!3m2!1sen!2sng!4v1685624248600!5m2!1sen!2sng',
};
<img
	width='150'
	height='150'
	src='https://img.icons8.com/ios-filled/150/000000/instagram-new--v1.png'
	alt='instagram-new--v1'
/>;
